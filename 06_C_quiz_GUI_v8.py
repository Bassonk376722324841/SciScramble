...
class Play:
    ...
    def new_round(self):
        ...
        if self.played == False and self.rounds_played != rounds:
        ...
        # Sends the user to the stats screen once the game ends
        elif self.played == False and self.rounds_played == rounds:
             Stats(self.successful_guesses, self.correct_answers, self.skipped)


        # If a user types nothing, or uses a space in their text, then the program sends this error
        if len(answer) == 0 and self.played == True:
            self.quiz_labels_ref[1].config(text="Please enter text without open spaces")
            self.played = False

        # Compare user answer with actual answer
        elif answer == self.true_terms[self.n].lower() and self.played == True:
             self.term_entry.delete(0, END)
